...
## 十一、Claude Code 与 OpenCode 压缩机制对比

### 1. 核心原理对比

| 特性 | Claude Code | OpenCode |
|------|------------|----------|
| 压缩方式 | 生成对话摘要，插入压缩边界标记 | 生成会话摘要，创建 compaction 类型消息 |
| 存储位置 | 原始会话文件（JSONL 格式） | 会话消息系统（作为特殊消息类型） |
| 触发方式 | 自动（token 阈值）+ 手动（/compact 命令） | 自动（overflow 检测）+ 手动 |
| 压缩策略 | 完全压缩 + 部分压缩（up_to/from） | 完全压缩 + 工具输出清理（prune） |

### 2. 技术实现对比

| 方面 | Claude Code | OpenCode |
|------|------------|----------|
| 缓存机制 | 提示缓存共享，减少重复计算 | 无明确缓存共享机制 |
| 插件支持 | 预压缩和后压缩钩子 | experimental.session.compacting 钩子 |
| 上下文重建 | 重新注入文件状态、技能使用等 | 通过插件机制注入自定义上下文 |
| 压缩边界 | 详细的 compact_boundary 元数据 | compaction 类型消息 + 摘要标记 |
| 错误处理 | 提示过长时自动截断并重试 | 会话过大时返回错误 |

### 3. 优势对比

#### Claude Code 优势
1. **更灵活的压缩策略**：支持部分压缩，可选择压缩对话的特定部分
2. **更完善的缓存优化**：通过缓存共享机制显著提高压缩效率
3. **更详细的元数据**：压缩边界包含丰富的元数据，便于会话管理
4. **更智能的上下文重建**：自动重新注入重要上下文，确保模型信息完整
5. **更强大的错误处理**：提示过长时自动截断并重试，提高压缩成功率

#### OpenCode 优势
1. **更简洁的实现**：压缩逻辑更集中，易于理解和维护
2. **更灵活的插件系统**：通过插件可完全自定义压缩提示和上下文
3. **工具输出清理**：prune 功能自动清理旧工具输出，节省上下文空间
4. **统一的消息系统**：压缩作为消息类型，与其他消息处理流程一致
5. **多语言支持**：内置多语言本地化支持
...
| 场景 | 推荐选择 | 原因 |
|------|----------|------|
| 长对话管理 | Claude Code | 更灵活的压缩策略和缓存优化 |
| 自定义压缩逻辑 | OpenCode | 更强大的插件系统 |
| 工具密集型会话 | OpenCode | 工具输出清理功能更有效 |
| 性能敏感场景 | Claude Code | 缓存共享机制减少 token 消耗 |
| 多语言环境 | OpenCode | 内置多语言支持 |

### 5. 总结

Claude Code 和 OpenCode 的压缩机制各有优势，选择哪种方案取决于具体的应用场景：

- **Claude Code** 适合需要处理长对话、对性能要求较高的场景，其缓存共享机制和灵活的压缩策略能够有效减少 token 消耗，提高压缩效率。

- **OpenCode** 适合需要高度自定义压缩逻辑、工具使用频繁的场景，其简洁的实现和强大的插件系统使得压缩过程更加可控和可扩展。

两种方案都实现了通过生成摘要来管理长对话的核心目标，但在具体实现细节和优化策略上各有特色，开发者可以根据实际需求选择合适的方案。
