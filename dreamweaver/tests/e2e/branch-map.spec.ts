...
test.describe('分支导图视图 (T-UI-006)', () => {
  ...
  test('应该显示分支节点', async ({ page }) => {
    ...
    await expect(page.locator('text=分支A: 接受邀请')).toBeVisible();
  ...
  test('视图切换按钮应该可点击', async ({ page }) => {
    ...
    // 验证按钮状态变化（通过样式类）
    const timelineButton = page.locator('text=时间线视图').locator('..');
    await expect(timelineButton).toHaveClass(/bg-surface-bright/);
    
    // 点击树状视图
    await page.click('text=树状视图');
    
    // 验证按钮状态变化
    const treeButton = page.locator('text=树状视图').locator('..');
    await expect(treeButton).toHaveClass(/bg-surface-bright/);
  });
  ...
  test('应该显示分支状态标签', async ({ page }) => {
    ...
    // 验证 IF-line 标签
    await expect(page.locator('text=IF-line')).toBeVisible();
    // 验证 archived 标签
