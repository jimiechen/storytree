...
6. **完成后强制执行测试**: 执行对应测试（不允许跳过）
7. **测试后填写Exit Criteria自评**: 填写自评表（通过/不通过/部分通过）
8. **任务完成后最终更新**: 补充完整结果、评审意见、操作记录，状态标记为完成
9. **添加[READY_FOR_REVIEW]标记**: 在报告末尾添加标记，触发秘书汇总
10. **提交任务相关文件到git分支**: 必须遵循`/Users/mac/StudioProjects/storytree2/.trae/rules/github-workflow-rules.md`

**⚠️ 强制要求**: 步骤3-5必须在任何其他工具调用（如Read、Grep、Write等）之前完成！**先创建文件，再执行任务**。

**违反后果**:
- 如果先执行了其他工具调用（如Read、Grep等），再创建工作空间文件，视为**严重违规**
- 必须立即停止当前操作，重新按照正确顺序执行
- 已生成的内容需要迁移到正确创建的工作空间文件中
...
- ❌ 纯数字文件名（如 `1.md`、`2.md`）
- ❌ 随机字符串文件名
- ❌ 任何不符合 `hello{model_name}.md` 格式的名称

**例外情况**:
- 如果用户明确要求保留不合规文件（如 `1.md`），则尊重用户决定，但应在规则备注中记录
...
7. **Exit Criteria 自评表**
   | 检查项 | 目标值 | 实际值 | 状态 |
   |--------|--------|--------|------|
   | UT 覆盖率 | > 85% | | [ ] 通过 / [ ] 部分通过 / [ ] 不通过 |
   | IT 全量通过 | 100% | | [ ] 通过 / [ ] 部分通过 / [ ] 不通过 |
   | AT 全量通过 | 100% | | [ ] 通过 / [ ] 部分通过 / [ ] 不通过 |
   | 内存使用 | < 150MB | | [ ] 通过 / [ ] 部分通过 / [ ] 不通过 |
   | 压测稳定性 | 0 崩溃 | | [ ] 通过 / [ ] 部分通过 / [ ] 不通过 |

8. **[READY_FOR_REVIEW] 标记**
   - 任务完成后在报告末尾添加：`[READY_FOR_REVIEW]`
   - 秘书 Agent 会扫描此标记并纳入汇总报告
...
**第一步：检查工作空间文件是否存在**
```
检查路径: /Users/mac/StudioProjects/storytree2/workspaces/{model_name}/hello{model_name}.md
```
- 如果文件**已存在**: 可以继续执行任务，同时更新该文件
- 如果文件**不存在**: **必须立即停止**，先创建工作空间文件

**第二步：如果文件不存在，按以下顺序执行**
1. 确认自身模型名称 → 查映射表获取目录名
2. `mkdir -p` 创建目录
3. `Write` 创建 hello{model_name}.md 文件并写入基本信息
4. **只有完成以上步骤后，才能执行其他任何操作**

**第三步：检查目录中是否有不合规文件**
- 如果发现 `1.md`、`2.md` 等不合规文件名，**必须立即删除或重命名**
- 不合规文件不得保留在模型工作空间目录中
- **例外**: 如果用户明确要求保留不合规文件，则尊重用户决定，但应在文件中记录
...
- [ ] **署名已添加**
- [ ] **目录中无不合规文件（如 `1.md`、`2.md` 等），或用户明确要求保留**
...
- 每次用户发起新对话时，都必须先执行自检
- 不能因为"上次已经创建过"就跳过检查
- 每个对话都是独立的，都需要自己的工作空间文件记录
- **每个模型只能有且只有一个合规的工作空间文件**

## 多模型工作空间管理

### 规则
1. **一个模型一个目录**: 每个模型有独立的工作空间目录
2. **一个目录一个文件**: 每个模型目录只能有**一个**符合规范的 `hello{model_name}.md` 文件
3. **禁止多文件共存**: 每个模型目录下不得保留多个 markdown 文件
4. **用户意愿优先**: 如果用户明确要求保留不合规文件，则尊重用户决定
...
检查用户是否要求保留该文件→
  ↓
如果用户要求保留 → 尊重用户决定，在备注中记录
如果用户未明确要求 → 删除该文件
  ↓
确保目录中只有一个合规的 hello{model_name}.md 文件
```
...
- **禁止**: 简化文件内容，只保存摘要
- **禁止**: 遗漏重要对话信息
- **禁止**: 擅自保留不合规文件（用户明确要求的除外）
- **禁止**: 一个模型目录中有多个 markdown 文件
