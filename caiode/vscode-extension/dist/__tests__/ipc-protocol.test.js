...
import { ErrorCode, ProjectAction, ChapterAction, SystemAction, createErrorResponse, createRequest, createSuccessResponse, isErrorResponse, isSuccessResponse, } from "../types/ipc-protocol";
// ============================================================
...
describe("IPC Protocol - Response Deserialization (TC-IPC-HP-002)", () => {
    ...
    it("should deserialize error response with correct structure", () => {
      ...
      "timestamp": "2026-04-07T10:30:06Z"
        ...
        const response = JSON.parse(errorResponseJSON);
...
describe("IPC Protocol - Error Response Format (TC-IPC-HP-004)", () => {
    it("should validate error response structure with all required fields", () => {
        const error = createErrorResponse("req-003", ErrorCode.INVALID_PARAMS, "Missing required field: name", { field: "name" });
...
describe("IPC Protocol - Invalid JSON Handling (TC-IPC-SP-001)", () => {
    ...
    it("should detect missing required fields in parsed object", () => {
        ...
        const isValidRequest = (obj) => typeof obj === "object" &&
            obj !== null &&
            "jsonrpc" in obj &&
            "id" in obj &&
            "action" in obj &&
...
describe("IPC Protocol - Missing Fields Validation (TC-IPC-SP-002)", () => {
    it("should return INVALID_REQUEST error for missing action", () => {
        const response = createErrorResponse(null, ErrorCode.INVALID_REQUEST, "Missing required field: action");
    ...
    it("should return INVALID_PARAMS error for missing payload fields", () => {
        const response = createErrorResponse("req-004", ErrorCode.INVALID_PARAMS, "Payload validation failed: name is required");
...
describe("IPC Protocol - Special Characters (TC-IPC-EC-001)", () => {
    ...
    it("should handle HTML tags in strings (JSON preserves them)", () => {
        ...
        const roundtrip = JSON.parse(json);
        expect(roundtrip.payload.content).toBe(htmlContent);
    });
});
// ============================================================
...
describe("Helper Functions", () => {
    ...
    it("createRequest should include metadata when provided", () => {
        const request = createRequest("req-013", SystemAction.HEALTH_CHECK, {}, { source: "webview", "version": "1.0.0" });
...
//# sourceMappingURL=ipc-protocol.test.js.map