 ...
 */
/** Router event types */
export var RouterEvent;
(function (RouterEvent) {
    RouterEvent["REQUEST_RECEIVED"] = "request:received";
    RouterEvent["ROUTE_MATCHED"] = "route:matched";
    RouterEvent["HANDLER_START"] = "handler:start";
    RouterEvent["HANDLER_COMPLETE"] = "handler:complete";
    RouterEvent["HANDLER_ERROR"] = "handler:error";
    RouterEvent["RESPONSE_SENT"] = "response:sent";
    RouterEvent["MIDDLEWARE_ABORT"] = "middleware:abort";
...
export class MessageRouter {
    ...
    on(action, handler, options) {
        ...
        this.handlers.set(action, { handler: handler, options: options ?? {} });
        return this;
    }
    /**
    ...
    async processMessage(rawMessage) {
        ...
        try {
            ...
            if (this.isNotification(request)) {
                ...
                return createSuccessResponse(null, null); // No response expected
            }
            ...
            this.log("Unhandled error in processMessage", { error: error.message });
            return createErrorResponse(null, ErrorCode.INTERNAL_ERROR, "Internal server error");
        }
    }
    /**
    ...
    async processRequest(request, context) {
        ...
        for (const middleware of this.beforeMiddlewares) {
            ...
            if (result === null) {
                ...
                return createErrorResponse(request.id, ErrorCode.INTERNAL_ERROR, "Request aborted by middleware");
            }
        ...
        if (!route) {
            if (this.config.strictMode) {
                return createErrorResponse(request.id, ErrorCode.METHOD_NOT_FOUND, `Unknown action: ${currentRequest.action}`);
            }
            this.log(`No handler for action: ${currentRequest.action}`, { action: currentRequest.action });
            return createErrorResponse(request.id, ErrorCode.METHOD_NOT_FOUND, `Action not found: ${currentRequest.action}`);
        }
        this.emit({
    ...
    async handleNotification(notification, context) {
        ...
        if (route) {
            try {
                await Promise.resolve(route.handler(notification, context));
            }
            catch (error) {
    ...
    async applyAfterMiddlewares(response, request, context) {
    ...
    defaultErrorHandler(error, request, _context) {
        this.log("Handler error", {
            action: request.action,
            error: error.message,
            stack: error.stack,
        });
        return createErrorResponse(request.id, ErrorCode.INTERNAL_ERROR, error.message || "Unknown error");
    }
    emit(data) {
